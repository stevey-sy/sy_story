...
<div class = "wrap">
    ...
    <table>
        ...
        if (!isset($_GET['priority'])) {
            $sql = mq("select * from products where $category like '%$search_con%'");
        } else if (!isset($_GET['search'])){
            $sql = mq("select * from products ");
        } else if (isset($_GET['category']) && isset($_GET['search']) && isset($_GET['priority'])) {
            $sql = mq("select * from products where $category like '%$search_con%'");
        }
        ...
        <table class="list_table">
            ...
            if(!isset($_GET['search'])) {
                ...
                $query="SELECT * FROM products WHERE $category LIKE '%$search_con%' ORDER BY $priority DESC limit 5";
            }
...
//                $query="SELECT * FROM products ORDER BY $priority DESC limit $start_num, $list_limit_per_page";
//
//            } else if (isset($_GET['catgo']) && isset($_GET['search']) && isset($_GET['priority'])) {
//                $query="SELECT * FROM products WHERE $category LIKE '%$search_con%' ORDER BY $priority DESC limit $start_num, $list_limit_per_page";
//            }
        ...
        <div id="page_num">
            <ul>
                ...
                { // 만약 page가 1보다 작거나 같다면 처음 페이지
                    ...
                    echo "<li><a href='?category=$category&search=$search_con&priority=$priority&page=1'>처음</a></li>";
                }
                if($page <=1)
                {
                    ...
                    echo "<li><a href='?category=$category&search=$search_con&priority=$priority&page=$pre'>이전</a></li>";
                }
                for($i=$block_start; $i<=$block_end; $i++){
                    ...
                    if($page == $i){ //만약 page가 $i와 같다면
                        ...
                        echo "<li><a href='?category=$category&search=$search_con&priority=$priority&page=$i'>[$i]</a></li>"; //아니라면 $i
                    }
                }
                if($block_num >= $total_block){ //만약 현재 블록이 블록 총개수보다 크거나 같다면 빈 값
                    ...
                    echo "<li><a href='?category=$category&search=$search_con&priority=$priority&page=$next'>다음</a></li>";
                }
                if($page >= $total_page){ //만약 page가 페이지수보다 크거나 같다면
                    echo "<li class='mark_red'>마지막</li>"; //마지막 글자에 긁은 빨간색을 적용한다.
                }else{
                    echo "<li><a href='?category=$category&search=$search_con&priority=$priority&page=$total_page'>마지막</a></li>"; //아니라면 마지막글자에 total_page를 링크한다.
                }
                ?>
            </ul>
        </div>
